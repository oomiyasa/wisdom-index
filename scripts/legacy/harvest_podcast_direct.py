#!/usr/bin/env python3
"""
Direct podcast transcript harvesting from specific sources
"""
...
def harvest_direct_podcast_transcripts() -> List[Dict[str, Any]]:
    """Harvest transcripts from specific podcast sources"""
    
    # Direct transcript sources
    transcript_sources = [
        {
    ...
    for source in transcript_sources:
        try:
            ...
            if source['type'] == 'blog':
                source_rows = _harvest_blog_transcripts(source, search_keywords)
            elif source['type'] == 'website':
                source_rows = _harvest_website_transcripts(source, search_keywords)
            elif source['type'] == 'npr':
                source_rows = _harvest_npr_transcripts(source, search_keywords)
...
def _harvest_blog_transcripts(source: Dict, search_keywords: List[str]) -> List[Dict[str, Any]]:
    """Harvest from blog-style podcast sites"""
    rows = []
...
def _harvest_website_transcripts(source: Dict, search_keywords: List[str]) -> List[Dict[str, Any]]:
    """Harvest from podcast websites"""
    rows = []
    ...
    try:
        ...
        print(f"     ⚠️  Website harvest failed: {e}")
    
    return rows

def _harvest_npr_transcripts(source: Dict, search_keywords: List[str]) -> List[Dict[str, Any]]:
    """Harvest from NPR-style sites"""
    rows = []
    ...
    try:
        ...
        print(f"     ⚠️  NPR harvest failed: {e}")
...
def main():
    """Test direct podcast transcript harvesting"""
    print("🔍 Testing direct podcast transcript harvesting...")
    
    rows = harvest_direct_podcast_transcripts()
    
    print(f"✅ Found {len(rows)} insights")
    
    if rows:
        output_file = "data/podcast_direct.csv"
